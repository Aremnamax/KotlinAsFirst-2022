...
fun main() {
    ...
    println(triangleKind(7.070541216611483, 7.070541216611483, 0.1))
...
fun rookOrBishopThreatens(
    ...
    val kbxx = abs(kingX - bishopX)
    val kbyy = abs(kingY - bishopY)
    return when {
        (kingX == rookX && kbxx == kbyy) || (kingY == rookY && kbxx == kbyy) -> 3
...
fun triangleKind(a: Double, b: Double, c: Double): Int = when {
    a > b + c || c > b + a || b > a + c -> -1
    sqr(max(max(a, b), c)) < sqr(min(min(a, b), c)) + sqr(a + b + c - max(max(a, b), c) - min(min(a, b), c)) -> 0
    sqr(max(max(a, b), c)) == sqr(min(min(a, b), c)) + sqr(a + b + c - max(max(a, b), c) - min(min(a, b), c)) -> 1
    else -> 2
}
...
fun segmentLength(a: Int, b: Int, c: Int, d: Int): Int {
    return if ((a <= c && (c <= b) && b <= d) || (c <= a && (a <= d) && d <= b) || (c <= a && (a <= b) && b <= d) ||
        (a <= c && (c <= d) && d <= b)
    ) {
        when {
            (a <= c && (c <= b) && b <= d) -> (b - c)
